...
BUNDLE_ID="com.luke.paster"
NOTARY_PROFILE="Paster"
GITHUB_REPO="lukehsuhao/paster"

APP_DIR="Paster.app/Contents"
DMG_NAME="Paster-${VERSION}.dmg"
ZIP_NAME="Paster-${VERSION}.zip"

echo "========================================="
echo "  Paster v${VERSION} Release"
echo "========================================="

# ── 1. 更新版本號 ──
echo ""
echo "📝 Step 1: 更新版本號為 ${VERSION}..."
sed -i '' "s/let currentVersionString = \".*\"/let currentVersionString = \"${VERSION}\"/" Sources/Paster/Core/UpdateManager.swift
...
cp .build/apple/Products/Release/Paster "$APP_DIR/MacOS/Paster"
...
echo "   架構: $(file "$APP_DIR/MacOS/Paster" | sed 's/.*: //')"
...
rm -f "$ZIP_NAME"
ditto -c -k --keepParent Paster.app "$ZIP_NAME"
...
xcrun stapler staple Paster.app
echo "   ✅ Staple 完成"
...
# 也製作 DMG
rm -f "$DMG_NAME"
hdiutil create -volname "Paster" \
    -srcfolder Paster.app \
    -ov -format UDZO \
    "$DMG_NAME"
